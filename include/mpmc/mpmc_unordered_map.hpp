#ifndef MPMC_UNORDERED_MAP_HPP
#define MPMC_UNORDERED_MAP_HPP

/**
 * @file mpmc_unordered_map.hpp
 * @brief Lock-free MPMC hash map with a std::unordered_map-compatible API.
 *
 * The underlying engine is the split-ordered list
 * (detail/split_ordered_table.hpp): all entries live in one lock-free
 * sorted list, and the bucket array is only an index of lazily created
 * dummy nodes, so insert, erase, find and resize are all lock-free.
 *
 * API notes (deviations from std::unordered_map are documented here and in
 * docs/API_en.md):
 *
 *   * Iterators are weakly consistent (Java ConcurrentHashMap style): a
 *     dereference re-locates the element, concurrent inserts and erases
 *     are tolerated, and an element erased while iterating is simply
 *     skipped. Using a reference after the element was erased is
 *     undefined.
 *   * swap(), hint-emplace and heterogeneous lookup are not provided
 *     (not safely definable for a lock-free container).
 *   * clear() requires exclusive access (no concurrent operations).
 *   * The Hash and KeyEqual functors must not throw and must be safe for
 *     concurrent use; the key type must be nothrow-copy-constructible and
...
namespace mpmc
{
    ...
    class unordered_map
    {
        // The Hash and KeyEqual functors must not throw and must be safe
        // for concurrent use; std::hash / std::equal_to satisfy this. The
        // requirement is documented (not static-asserted: the standard
        // library functors are not noexcept-specified).
        static_assert(std::is_nothrow_copy_constructible<Key>::value,
                      "mpmc::unordered_map requires a nothrow-copy-"
                      "constructible key");
        static_assert(std::is_nothrow_destructible<Key>::value,
                      "mpmc::unordered_map requires a nothrow-destructible "
                      "key");
        static_assert(std::is_nothrow_destructible<T>::value,
                      "mpmc::unordered_map requires a nothrow-destructible "
                      "mapped type");

        using engine = detail::split_ordered_table<Key, T, Hash, KeyEqual,
    ...
    public:
        ...
        // ---- Construction ---------------------------------------------------
        /**
         * @brief Construct an empty map with the default hasher, key
        ...
        /**
         * @brief Construct an empty map with a specific allocator.
         *
        ...
        /**
         * @brief Construct a map with a requested bucket count and
        ...
        /**
         * @brief Construct a map with a requested bucket count and a
        ...
        unordered_map(size_type n, const allocator_type& alloc)
            : unordered_map(n, hasher(), key_equal(), alloc)
        {
        }

        /**
         * @brief Construct a map with a requested bucket count, a custom
        ...
        unordered_map(size_type n, const hasher& hf, const allocator_type& alloc)
            : unordered_map(n, hf, key_equal(), alloc)
        {
        }
        ...
        std::pair<iterator, bool> emplace(Args&&... args)
        {
            ...
            // Copy the key out before the node can be destroyed.
            const Key k = n->key();
         ...
         *        already present.
         *
         * Equivalent to emplace(std::forward<P>(p)). O(1) expected time.
         *
         ...
         * it is already present, so the key is never moved-from. O(1)
         * expected time.
         *
        ...
        std::pair<iterator, bool> insert_or_assign(const key_type& k, M&& obj)
        {
            ...
            if (!r.second)
            {
                // present: assign (the reference is weakly consistent, but
                // the assignment itself is safe)
                const std::uint64_t mixed = impl_.mixed_hash(k);
                value_type* p = impl_.find_value_ptr(k, mixed,
                                                     impl_.so_key_of(mixed));
                if (p != nullptr)
                {
        ...
        mapped_type& operator[](const key_type& k)
        {
            const std::uint64_t mixed = impl_.mixed_hash(k);
            const std::uint64_t sk = impl_.so_key_of(mixed);
            value_type* p = impl_.find_value_ptr(k, mixed, sk);
            if (p != nullptr)
            {
                return p->second;
            }
            ...
            engine::destroy_node(n);
            p = impl_.find_value_ptr(k, mixed, sk);
            return p->second;
        }
        ...
        mapped_type& at(const key_type& k)
        {
            ...
            if (p == nullptr)
            {
                throw std::out_of_range("mpmc::unordered_map::at");
            }
            return p->second;
        }
        ...
        private:
            ...
            explicit iterator(const unordered_map* owner)
                : owner_(owner)
                , is_end_(true)
                , mixed_(0)
                , sk_(0)
            ...
            iterator(const unordered_map* owner, const Key& k,
        ...
        private:
            ...
            explicit const_iterator(const unordered_map* owner)
                : owner_(owner)
                , is_end_(true)
                , mixed_(0)
                , sk_(0)
            ...
            const_iterator(const unordered_map* owner, const Key& k,
                           std::uint64_t mixed, std::uint64_t sk)
                : owner_(owner)
                , is_end_(false)
                , key_(k)
